...
$oss_sdk_service = new ALIOSS(OSS_ACCESS_ID,OSS_ACCESS_KEY,OSS_DOMAIN);
...
if(isset($argv[1])){
	...
	var_dump($param);
	$param = array_merge(array($oss_sdk_service),$param);
	var_dump($param);
	$result = call_user_func_array($argv[1],$param);
	var_dump($result);
}
try{
...
function set_bucket_acl($obj,$bucket,$acl){
	...
	empty($acl) and $acl = ALIOSS::OSS_ACL_TYPE_PRIVATE;
	//$acl = ALIOSS::OSS_ACL_TYPE_PUBLIC_READ;
	//$acl = ALIOSS::OSS_ACL_TYPE_PUBLIC_READ_WRITE;
...
//通过multipart上传整个目录
function upload_by_dir($obj,$bucket,$dir){
	$bucket = 'icodeu';
...
function batch_upload_file($obj,$bucket='',$object='',$directory='',$exclude='.|..|.svn',$recursive=false){
	if(empty($bucket)||empty($object)||empty($directory)){
		return false;
	}
	$options = array(
		'bucket' 	=> $bucket,
