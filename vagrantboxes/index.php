...
<a href="https://github.com/garethr/vagrantboxes-heroku"><img style="position: absolute; top: 0; right: 0; border: 0;"
...
<div id="main">
    ...
    <p><a href="http://vagrantup.com">Vagrant</a> is an amazing tool for managing virtual machines via a simple to use
        command line interface. With a simple <code>vagrant up</code> you can be working in a clean environment based on
        a standard template.</p>

    <p>These standard templates are called <a href="http://docs.vagrantup.com/v2/boxes.html">base boxes</a>, and this
        website is simply a list of boxes people have been nice enough to make publicly available.</p>

    <h2>Suggest a Box</h2>

    <p>Do you know of another base box? <a href="http://github.com/garethr/vagrantboxes-heroku">Send a pull request</a>
        and we'll add it to the list below.</p>
...
<pre>
 ...
 <span>$ </span>vagrant up
        ...
        foreach ($html->find('#dataTable tr') as $tr) {
            ...
            foreach ($tr->find('td') as $td) {
                ...
                if ($i == 3) {
                    ...
                    $file_key = md5($url);
                    if (isset($url_array[$file_key]) and isset($url_array[$file_key][1]) and strlen($url_array[$file_key][1])) {
                        $tr_str .= '<td class="baidu_disk">'.$url_array[$file_key][1].'</td>' . PHP_EOL;
                    } else {
                        $tr_str .= '<td class=""></td>' . PHP_EOL;
                    }
                }
            }
            $tr_str .= '</tr>' . PHP_EOL;
    ...
    <div id="footer">
        <p>A project from <a href="http://morethanseven.net">Gareth Rushgrove</a>. The design of this site is heavily
            inspired by the lovely <a href="http://vagrantup.com">Vagrant</a> design by <a
                href="http://www.simonecarletti.com/">Simone Carletti</a>.</p>
    </div>
